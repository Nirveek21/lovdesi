...
exports.cuponsData = async (params, next) => {
    logger.info("*** Starting %s of %s ***", getName().functionName, getName().fileName);
    try {
        params = params.data;
        let dataExist = await db.Cupon.findOne({ where: { name: params.name } });
        if (dataExist) {
            ...
            let results = await db.Cupon.create(params);
            logger.info("*** Ending %s of %s ***", getName().functionName, getName().fileName);
...
exports.cuponsListData = async (params, next) => {
    logger.info("*** Starting %s of %s ***", getName().functionName, getName().fileName);
    try {
        params = params.data;
        var inner_params = {};
        ...
        if (params.filter) {
            if (Object.keys(params.filter).length > 0) {
                ...
                let TemplateData = db.Cupons.rawAttributes;
        ...
        let result = await db.Cupon.findAll(findAllData);
        if (!params.filter || (params.filter && (!params.filter.total || parseInt(params.filter.total) == 0))) {
            ...
            let count = await db.Cupon.findAll(findAllData);
            logger.info("*** Ending %s of %s ***", getName().functionName, getName().fileName);
...
exports.cuponsUpdateData = async (params, userId, org_id, next) => {
    logger.info("*** Starting %s of %s ***", getName().functionName, getName().fileName);
    try {
        let id = params.id;
        params = params.data;
        let dataExist = await db.Cupon.findOne({ where: { id: id } });
        if (dataExist != null) {

            let results = await db.Cupon.update(params, { where: { id: id } });
            if (results[0] == 0) {
                logger.error("*** Error in %s of %s ***", getName().functionName, getName().fileName);
...
exports.cuponsDeleteData = async (params) => {
    logger.info("* Starting %s of %s *", getName().functionName, getName().fileName);
    try {
        ...
        for (var i = 0; i < id.length; i++) {
            var dataExists = await db.Cupon.findOne({
                where: { id: id[i] }
            });
            if (dataExists == null) {
                ...
                var result = await db.Cupon.destroy({
                    where: { id: id[i] }
                });
                if (result > 0) {
                    var _hash = { "success": true };
                    _hash["data"] = { "id": id[i] };
                    _hash["message"] = `Cupons deleted successfully`;
                    _arr.push(_hash);
                } else {
                    logger.error("* Error in %s of %s *", getName().functionName, getName().fileName);
