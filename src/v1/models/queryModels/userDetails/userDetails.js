...
exports.userDetailsData = async (params, next) => {
    ...
    try {
        params = params.data;
        // let dataEx = await db.userDetails.findOne({ where: { name: params.name, org_id: org_id } });
        // if (!dataEx) {
        let results = await db.userDetails.create(params);
    ...
    try {
        ...
        if (params.filter) {
            if (Object.keys(params.filter).length > 0) {
                ...
                let TemplateData = db.userDetails.rawAttributes;
...
exports.userDetailsUpdateData = async (params, userDetailsId, org_id, next) => {
    logger.info("*** Starting %s of %s ***", getName().functionName, getName().fileName);
    try {
        let id = params.id;
        params = params.data;
        ...
        if (dataExist != null) {
            //let duplicate_checking = await db.userDetails.findOne({ where: { name: params.name } });
            // if (duplicate_checking != null) {
            //     logger.error("*** Error in %s of %s ***", getName().functionName, getName().fileName);
            //     return ({ success: false, message: 'store already exist', status: 409 })
            // } else {
            let results = await db.userDetails.update(params, { where: { id: id } });
            if (results[0] == 0) {
                logger.error("*** Error in %s of %s ***", getName().functionName, getName().fileName);
