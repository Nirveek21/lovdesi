...
exports.userData = async (params, next) => {
    logger.info("*** Starting %s of %s ***", getName().functionName, getName().fileName);
    try {
        params = params.data;
        // let dataEx = await db.User.findOne({ where: { name: params.name, org_id: org_id } });
        // if (!dataEx) {
        let results = await db.User.create(params);
        let password = await bcrypt.hash(createTempPass(8), saltRounds)
        let user_secret_info = {
            user_id:results.id,
            temp_password:password,
            status:"temp_password"
        }
        await db.UserSecretInfo.create(user_secret_info)
...
exports.userListData = async (params, next) => {
    logger.info("*** Starting %s of %s ***", getName().functionName, getName().fileName);
    try {
        params = params.data;
        var inner_params = {};
        ...
        if (params.filter) {
            if (Object.keys(params.filter).length > 0) {
                ...
                let TemplateData = db.User.rawAttributes;
        ...
        let result = await db.User.findAll(findAllData);
        if (!params.filter || (params.filter && (!params.filter.total || parseInt(params.filter.total) == 0))) {
            ...
            let count = await db.User.findAll(findAllData);
            logger.info("*** Ending %s of %s ***", getName().functionName, getName().fileName);
...
exports.userUpdateData = async (params, userId, org_id, next) => {
    logger.info("*** Starting %s of %s ***", getName().functionName, getName().fileName);
    try {
        let id = params.id;
        params = params.data;
        let dataExist = await db.User.findOne({ where: { id: id } });
        if (dataExist != null) {
            //let duplicate_checking = await db.User.findOne({ where: { name: params.name } });
            // if (duplicate_checking != null) {
            //     logger.error("*** Error in %s of %s ***", getName().functionName, getName().fileName);
            //     return ({ success: false, message: 'store already exist', status: 409 })
            // } else {
            let results = await db.User.update(params, { where: { id: id } });
            if (results[0] == 0) {
                logger.error("*** Error in %s of %s ***", getName().functionName, getName().fileName);
...
exports.userDeleteData = async (params) => {
    logger.info("* Starting %s of %s *", getName().functionName, getName().fileName);
    try {
        ...
        for (var i = 0; i < id.length; i++) {
            ...
            if (dataExists == null) {
                ...
                if (result > 0) {
                    ...
                    _hash["data"] = { "id": id[i] };
                    _hash["message"] = `User deleted successfully`;
                    _arr.push(_hash);
                } else {
                    logger.error("* Error in %s of %s *", getName().functionName, getName().fileName);
